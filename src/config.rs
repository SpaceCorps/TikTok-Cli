...
pub struct AccountConfig {
    /// Whatever identity was reported when the key was probed/verified (e.g. username/email or masked key).
    pub identity: String,
    pub added_at: String,
}
...
pub fn config_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("TIKTOK_CONFIG_DIR").filter(|v| !v.is_empty()) {
        return PathBuf::from(dir);
    }

    #[cfg(windows)]
    let dir = std::env::var_os("APPDATA").map(PathBuf::from).unwrap_or_default().join("tiktok-cli");

    #[cfg(not(windows))]
    let dir = {
        let home = std::env::var_os("HOME").map(PathBuf::from).unwrap_or_default();
        if cfg!(target_os = "macos") {
            home.join("Library").join("Application Support").join("tiktok-cli")
        } else {
...
pub fn load() -> Result<Config> {
    ...
    serde_norway::from_str::<Option<Config>>(&text).map(Option::unwrap_or_default).map_err(|e| {
        Error::other(format!("{} is not valid.", path.display()))
            .detail(e.to_string())
            .fix(format!("Fix or delete {}, then run: tiktok accounts add <name>", path.display()))
    })
}
        ...
        match file.try_lock() {
            ...
            Err(fs::TryLockError::WouldBlock) => {
                return Err(Error::other(format!(
                    "Timed out waiting for the lock at {}. Another tiktok process may be stuck.",
                    path.display()
                )));
            }
