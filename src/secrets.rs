...
//! refuses to start rather than silently writing a file - `TIKTOK_ALLOW_PLAINTEXT_STORE=1` is
...
/// The backend for this process. Cached: changing `TIKTOK_SECRET_STORE` mid-process has no
...
fn build() -> Result<Store> {
    if let Some(forced) = std::env::var("TIKTOK_SECRET_STORE").ok().filter(|s| !s.trim().is_empty()) {
        return match forced.to_lowercase().as_str() {
            ...
            _ => Err(Error::invalid(format!(
                "TIKTOK_SECRET_STORE='{forced}' is not a backend available on this platform."
            ))
            .fix("Unset TIKTOK_SECRET_STORE, or set it to one of: dpapi, keychain, libsecret, plaintext.")),
        };
    }
...
fn fallback(reason: &str) -> Result<Store> {
    if std::env::var("TIKTOK_ALLOW_PLAINTEXT_STORE").as_deref() == Ok("1") {
        return Ok(Store::Plaintext);
    }
    Err(Error::other("No secure credential store is available on this machine.")
        .detail(reason)
        .fix("Install a keystore, or set TIKTOK_ALLOW_PLAINTEXT_STORE=1 to store API keys in a 0600 file instead."))
}
...
impl Store {
    /// Backend name, as reported by `tiktok accounts list`.
...
fn warn_plaintext() {
    ...
    WARNED.get_or_init(|| {
        eprintln!(
            "warning: API keys are stored unencrypted in {} (TIKTOK_ALLOW_PLAINTEXT_STORE=1).",
    ...
    pub fn unprotect(data: &[u8], path: &Path) -> Result<Vec<u8>> {
        ...
        if ok == 0 {
            return Err(Error::other("The stored API keys could not be decrypted.")
                ...
                .fix(format!("Delete {} and run: tiktok accounts add <name>", path.display())));
        }
        Ok(take(out))
    }
}
